...
PUBLIC void mem_task()
{
	...
	/* First wait for a request to read or write. */
		receive(ANY, &mess);
		if (mess.m_source < 0){
			printk("mem task got message from ", mess.m_source);
			continue;
		}
...
/* Read or write /dev/null, /dev/mem, /dev/kmem, or /dev/ram. */
	...
	//user_phys = umap((vir_bytes) m_ptr->ADDRESS, (vir_bytes) count);
	user_phys = getmesgphys(rp,(vir_bytes) m_ptr->ADDRESS);
	if (user_phys == 0) return(E_BAD_ADDR);

	//printk("mem_phys:%x user_phys:%x\n",mem_phys,user_phys);
	/* Copy the data. */
