...
PRIVATE char winibuf[BLOCK_SIZE];  
...
/* Carry out a read or write request from the disk. */
	...
	//	return(EOF);
	  ...
	  wn->wn_address = getmesgphys(proc_addr(m_ptr->PROC_NR),(vir_bytes) m_ptr->ADDRESS);
...
PUBLIC void init_win()
{
	nr_drives = (u8*)(0x475); /* Get the nummer of drives from the bios */
