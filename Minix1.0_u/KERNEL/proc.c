...
PUBLIC void cp_mess(int src,struct s_proc *rpsrc,u32 *src_addr,struct s_proc *rpdst,u32 *dst_addr);
...
PUBLIC void interrupt(int task, message *m_ptr){
 ...
 PUBLIC phys_bytes getmesgphys(struct s_proc *rp,u32 msg_vir);
/*===========================================================================*
 *				sys_call				     * 
 *===========================================================================*/
 ...
 */
	...
	if (src_dest < -NR_TASKS || (src_dest >= NR_PROCS && src_dest != ANY) ) {
		rp->regs.eax = E_BAD_SRC;
		return;
	}
    

	if (function != BOTH && caller >= LOW_USER) {
		rp->regs.eax = E_NO_PERM;	/* users only do BOTH */
		//return; debug use for current 
	}

	/* The parameters are ok. Do the call. */
	if (function & SEND) {
		n = mini_send(caller, src_dest, m_ptr);	/* func = SEND or BOTH */
		if (function == SEND || n != OK) rp->regs.eax = n;
 ...
 */
   ...
   // vb = (vir_bytes) m_ptr;
	...
	if ( (dest_ptr->p_flags & RECEIVING) &&
		...
		cp_mess(caller, caller_ptr, m_ptr, 
				dest_ptr, dest_ptr->p_messbuf);
 ...
 */
  ...
  struct s_proc *caller_ptr, *sender_ptr, *prev_ptr;
  int sender;
  ...
  while (sender_ptr != NIL_PROC) {
	...
	if (src == ANY || src == sender) {
		...
		cp_mess(sender, sender_ptr, sender_ptr->p_messbuf,
					caller_ptr, m_ptr);
 ...
 *===========================================================================*/
 ...
 PUBLIC void setuppage(void){
	...
	if(cur_proc == mem_proc || cur_proc < 0 ) 
		return;

	//set text seg
	pagedir = ( (proc_ptr->p_map[T].mem_vir << CLICK_SHIFT) & 0xFFC00000) >> 22;
	pagetable = ( (proc_ptr->p_map[T].mem_vir << CLICK_SHIFT) & 0x3FF000)>>12;
	len = proc_ptr->p_map[T].mem_len;
	mem_phys = proc_ptr->p_map[T].mem_phys << CLICK_SHIFT;

	//get page table address
	pagetableAddr = (u32)page_dir[pagedir]& 0xFFFFF000;

	//disp_str(" \n                                      pagetableAddr: ");
	//disp_int(pagetableAddr);

	for(i=0;i<len;i++){		 
		tmp = pagetableAddr[pagetable] & 0xfff;
		tmp += mem_phys;
		pagetableAddr[pagetable]= tmp;
		pagetable++;
		mem_phys+=4096;
	 }
	...
	//set stack seg
	pagedir = ( (proc_ptr->p_map[S].mem_vir << CLICK_SHIFT) & 0xFFC00000) >> 22;
	pagetable = ( (proc_ptr->p_map[S].mem_vir << CLICK_SHIFT) & 0x3FF000) >> 12;
	len = proc_ptr->p_map[S].mem_len;
	mem_phys = proc_ptr->p_map[S].mem_phys << CLICK_SHIFT;
 ...
 PUBLIC phys_bytes getmesgphys(struct s_proc *rp,u32 msg_vir){
	 ...
	 if(t<0) return msg_vir;

	 if( ((msg_vir>>CLICK_SHIFT) <= rp->p_map[T].mem_vir + rp->p_map[T].mem_len) &&  
		 ((msg_vir>>CLICK_SHIFT) >= rp->p_map[T].mem_vir) ){
		 seg = T;
	 }
	 else{
		 seg = S;
 ...
 void cp_mess(int src,struct s_proc *rpsrc,u32 *src_addr,struct s_proc *rpdst,u32 *dst_addr){
	 u32 *srcaddrphys;
	 u32 *dstaddrphys;
	 int t;

	 srcaddrphys = getmesgphys(rpsrc,src_addr);
	 dstaddrphys = getmesgphys(rpdst,dst_addr);

	  //此种情况失败，分析一下原因2015/7/25。
	  //phys_copy(&src, dstaddrphys, 4);
	 //phys_copy(srcaddrphys+1, dstaddrphys+1, sizeof(message) - 4);

	 _cp_mess(src,rpsrc,srcaddrphys,rpdst,dstaddrphys);
 }
