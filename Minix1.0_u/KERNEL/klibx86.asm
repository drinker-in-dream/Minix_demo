...
; ========================================================================
;                  void build_sig(char sig_stuff[],s_proc *rp,int sig);
; ========================================================================
build_sig:
	push ebp
	mov  ebp, esp
	...
	mov ebx,[ebp + 8]		; bx points to sig_stuff
	mov esi,[ebp + 12]		; si points to proc table entry
	mov eax,[ebp + 16]		; ax = signal number
	mov [ebx],eax		        ; put signal number in sig_stuff
	mov eax,[esi + EIPREG]		; ax = signalled process' PC
	mov [ebx + 4],eax		; put pc in sig_stuff
	mov eax,[esi + CSREG]	        ; ax = signalled process' cs
	mov [ebx + 8],eax		; put cs in sig_stuff
	mov eax,[esi + EFLAGSREG]	; ax = signalled process' PSW
	mov [ebx+12],eax		; put psw in sig_stuff
...
; ------------------------------------------------------------------------
; void phys_copy(u32 * source, u32 * destination, u32 * bytecount);
; ------------------------------------------------------------------------
phys_copy:
	push	ebp
	mov	ebp, esp
