...
//struct mproc *child;	/* tells which process is exiting */
{
  ...
  mm_reply(child->mp_parent, child->mp_pid, r, NIL_PTR);

  /* Release the memory occupied by the child. */
  s = child->mp_seg[T].mem_len + child->mp_seg[S].mem_len;
  free_mem(child->mp_seg[S].mem_phys, s);	/* free the memory */
...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  long parent_abs, child_abs,pstake_abs;
  ...
  */
  ...
  prog_clicks = (phys_clicks) rmp->mp_seg[T].mem_len + (phys_clicks) rmp->mp_seg[S].mem_len;
  ...
  pstake_abs = (long) rmp->mp_seg[S].mem_phys << CLICK_SHIFT;
  i = mem_copy(ABS, 0, pstake_abs, ABS, 0, child_abs, rmp->mp_seg[S].mem_len << CLICK_SHIFT);
  if ( i < 0) panic("do_fork can't copy");
  i = mem_copy(ABS, 0, parent_abs, ABS, 0, (child_abs + (rmp->mp_seg[S].mem_len << CLICK_SHIFT)), rmp->mp_seg[T].mem_len << CLICK_SHIFT);
  if ( i < 0) panic("do_fork can't copy");
  ...
  rmc->mp_parent = mm_who;		/* record child's parent */
  rmc->mp_seg[S].mem_phys = child_base;
  rmc->mp_seg[T].mem_phys = child_base + rmp->mp_seg[S].mem_len;
  rmc->mp_exitstatus = 0;
  rmc->mp_sigstatus = 0;
  ...
  sys_fork(mm_who, child_nr, rmc->mp_pid);
  tell_fs(FORK, mm_who, child_nr, 0);
  /* Report child's memory map to kernel. */
  sys_newmap(child_nr, rmc->mp_seg);
	/* Reply to child to wake it up. */
  mm_reply(child_nr, 0, 0, NIL_PTR);
 ...
 */
  ...
  children = 0;
  for (rp = &mproc[0]; rp < &mproc[NR_PROCS]; rp++) {
	if ( (rp->mp_flags & IN_USE) && rp->mp_parent == mm_who) {
		children++;
