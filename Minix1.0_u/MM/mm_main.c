...
/* Wait for the next message and extract useful information from it. */
  ...
  mm_who = mm_in.m_source;		/* who sent the message */
  if (mm_who < HARDWARE || mm_who >= NR_PROCS) panic("MM called by", mm_who);
...
/* Main routine of the memory manager. */
  ...
  while(TRUE){
	...
	mm_reply(mm_who, error, result2, res_ptr);
  }
}
