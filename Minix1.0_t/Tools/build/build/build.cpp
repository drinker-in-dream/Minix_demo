...
void main(){
	...
	//read init
	if( (fpIn = fopen("INIT.bin","rb+") ) == NULL ){
		printf("\n can not open file INIT.bin!");
		exit(0);
	}
	...
	for(i = 0;i < Ehdr.e_phnum; i++){
		...
		initsize = MAX(initsize,elf32_phdr[i].p_vaddr + elf32_phdr[i].p_memsz);
	}
	sizes[6]=base >> CLICK_SHIFT;
	sizes[7]=(initsize - base + (CLICK_SIZE -1)) >> CLICK_SHIFT;
	fclose(fpIn);
	...
	printf("\n sizes[6]=%X , sizes[7]=%X, sizes[6]sh=%X, sizes[7]sh=%X",sizes[6],sizes[7],sizes[6] << CLICK_SHIFT,sizes[7] << CLICK_SHIFT );
