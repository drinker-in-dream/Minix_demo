...
//int *arglist;
{
  ...
  while (*s != '\0') {
	...
	switch(*s) {
	    ...
	    case 'c':	k = *valp++; tty_putc(k); s++; continue;
	    case 's':	p = (char *) *valp++; 
			p1 = p;
			while(c = *p++) tty_putc(c); s++;
			if ( (k = w - (p-p1-1)) > 0) while (k--) tty_putc(' ');
			continue;
	    default:	tty_putc('%'); tty_putc(*s++); continue;
	}

	k = bintoascii(l, r, a);
	if ( (r = w - k) > 0) while(r--) tty_putc(' ');
	for (r = k - 1; r >= 0; r--) tty_putc(a[r]);
	s++;
  }
}
