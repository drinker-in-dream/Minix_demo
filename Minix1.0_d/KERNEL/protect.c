...
/* function decalare in this file */
PRIVATE void init_idt_desc(unsigned char vector, u8 desc_type, t_pf_int_handler handler, unsigned char privilege);
...
PUBLIC void init_prot()
{
	...
	for(i=0;i<NR_TASKS+LOW_USER;i++){
		init_descriptor(&gdt[selector_ldt>>3],
				vir2phys(seg2phys(SELECTOR_KERNEL_DS), proc[i].ldts),
...
{
	int i;
	int text_color = 0x74; /* gray background red text*/
	...
	/* clear the first 5 rows，and clear disp_pos  */
	disp_pos = 0;
	for(i=0;i<80*5;i++){
		disp_str(" ");
	}
