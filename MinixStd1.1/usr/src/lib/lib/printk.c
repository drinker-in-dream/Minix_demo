...
int *arglist;
{
  ...
  while (*s != '\0') {
	...
	switch(*s) {
	    ...
	    case 'c':	k = *valp++; ttyputc(k); s++; continue;
	    case 's':	p = (char *) *valp++; 
			p1 = p;
			while(c = *p++) ttyputc(c); s++;
			if ( (k = w - (p-p1-1)) > 0) while (k--) ttyputc(' ');
			continue;
	    default:	ttyputc('%'); ttyputc(*s++); continue;
	}

	k = bintoascii(l, r, a);
	if ( (r = w - k) > 0) while(r--) ttyputc(' ');
	for (r = k - 1; r >= 0; r--) ttyputc(a[r]);
	s++;
  }
}
